...
<template>
  <div class="flex items-center gap-2">
    <div class="relative h-8">
      <Search class="absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2" :color="store === 'light' ? 'rgb(82 82 91)' : 'rgb(113 113 122)'" />
      <UiInput
        v-model="model"
    ...
    >
      ...
      <span class="text-sm">{{ $route.path === localPath(NOTES_ROUTE) ? $t('top_menu.btns.note') : $t('top_menu.btns.favorite') }}</span>
    </UiButton>
  </div>
</template>
